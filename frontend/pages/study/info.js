...
import { deleteAPI, getUserAtStudy, changeStudyHost, getStudyByUserId, quitStudy } from "../../pages/api/study"
import * as studyActions from '../../store/module/study';

import Router from "next/router";

function StudyInfo(){
    let clubData = useSelector(({ study }) => study.myStudy);
    ...
    useLayoutEffect(() => {
        ...
        .then(res => {
            console.log(res)
            // To do : 스터디 여러개인 경우 처리
            dispatch(studyActions.setMyStudy({study: res.studies[0]}))
            dispatch(studyActions.setStudyDetail({detail: res.studies[0]}))
        });
    }, []);
