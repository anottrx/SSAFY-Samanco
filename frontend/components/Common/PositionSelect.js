...
function PositionSelect(props) {
  ...
  if (props.initData) {
    props.initData.map((data) => {
      if (
        data.position.includes('total') &&
        data.size > 0 &&
        data.position !== 'totalSize'
      ) {
  ...
  const handleAutocompleteChange = (event) => {
    ...
    if (!isInclude) {
      setPositions([...positions, { name: name, count: 1 }]);
    }
  };
  useEffect(() => {
    // 상위 컴포넌트에게 바뀐 포지션 전달
  ...
  return (
    <>
      <Tooltip
        title="본인의 포지션을 포함해 인원 수를 선택해주세요. "
        placement="top-end"
      >
        <Box>
          <Autocomplete
